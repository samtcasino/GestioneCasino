...
Quando è presente un errore in un campo, quest'ultimo diventa rosso. Però, quando si inserisce nuovamente un valore valido al suo interno, quest'ultimo deve ritornare al colore di partenza. Questo non accadeva ed è stato corretto impostando il colore di sfondo del campo a bianco con il seguente codice:
```javascript
