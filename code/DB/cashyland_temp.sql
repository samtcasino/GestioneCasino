CREATE DATABASE  IF NOT EXISTS `cashyland` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */;
USE `cashyland`;
...
CREATE TABLE `media` (
  ...
  UNIQUE KEY `room_location_UNIQUE` (`room_location`),
  KEY `fk_type_idx` (`type`),
  KEY `fk_game_type_idx` (`type`),
  CONSTRAINT `fk_media_game` FOREIGN KEY (`game_name`) REFERENCES `game` (`name`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_media_room` FOREIGN KEY (`room_location`) REFERENCES `room` (`location`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_media_type` FOREIGN KEY (`type`) REFERENCES `type` (`name`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2019-02-27 14:23:50
